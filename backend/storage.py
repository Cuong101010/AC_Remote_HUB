...
class Storage:
    ...
    def register_user(self, username, password, display_name=""):
        with self.lock:
            ...
            user = {
                ...
                "createdAt": time.time(),
                "deviceIds": []       # danh sách deviceId của user này
            }
    ...
    def _create_session_unlocked(self, user_id):
        ...
        self.sessions[token] = {
            "userId": user_id,
            "createdAt": time.time(),
            "expiresAt": time.time() + 30 * 24 * 3600  # 30 ngày
        }
        self._save_json(self.sessions_file, self.sessions)
        return token

    def verify_session(self, session_token):
        """Trả về user dict nếu token hợp lệ, None nếu không."""
        with self.lock:
    ...
    def assign_device_to_user(self, username, device_id):
        """Gán thiết bị cho user (sau khi pair)."""
        with self.lock:
            user = self.users.get(username)
            if not user:
                return False
    ...
    def register_device(self, reg_data):
        with self.lock:
            ...
            device_info = {
                ...
                "firmwareVersion": reg_data.get("firmwareVersion", "0.1.0"),
    ...
    def update_heartbeat(self, device_id, hb_data):
        with self.lock:
            ...
            dev["ssid"]                    = hb_data.get("ssid",  dev.get("ssid"))
            dev["rssi"]                    = hb_data.get("rssi",  dev.get("rssi"))
            dev["uptimeMs"]                = hb_data.get("uptimeMs",  dev.get("uptimeMs"))
            dev["freeHeap"]                = hb_data.get("freeHeap",  dev.get("freeHeap"))
            dev["learning"]                = hb_data.get("learning", False)
            dev["activeLearningProfileId"] = hb_data.get("activeLearningProfileId", "")
    ...
    def list_devices(self, user_device_ids=None):
        """Nếu user_device_ids không None, chỉ trả thiết bị thuộc user đó."""
        with self.lock:
            result = []
            now = time.time()
    ...
    def save_learned_signal(self, device_id, profile_id, signal_data):
        with self.lock:
            ...
            signals.append({
                ...
                "controlType":   signal_data.get("controlType"),
                "commonDecoded": signal_data.get("commonDecoded"),
                "commonState":   signal_data.get("commonState"),
    ...
    def set_learning_timeout(self, device_id, profile_id):
        with self.lock:
            if device_id in self.devices:
                self.devices[device_id]["learning"] = False
                self.devices[device_id]["activeLearningProfileId"] = ""
